...
//感谢https://gitee.com/vsiryxm/play-substrate-node-template/blob/kitties-4.0.0-dev/pallets/kitties/src/lib.rs
...
fn create_failed_when_not_enough_balance_for_staking() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::create(Origin::signed(account_id)), Error::<Test>::TokenNotEnough);
...
fn transfer_failed_when_new_owner_not_enough_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::transfer(Origin::signed(account_id), 0, 3), Error::<Test>::TokenNotEnough);
	});
}
...
fn breed_failed_when_not_enough_balance_for_staking() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::breed(Origin::signed(3u64), 0, 1), Error::<Test>::TokenNotEnough);
	});
}
